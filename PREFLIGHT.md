...
- content HEAD: `6e41587d976d0e02a30aef67a7aa7cc1c9c4254f`（既存決定論コアを正本とする`meta-security-run-bundle/v1`、JSON descriptor fail-closed、独立implementation SHA照合、seed・event container境界、115件回帰）
...
| 決定論・AI・UI境界テスト | pass | `npm test`: 115件pass。提案・receipt・PDCAに加え、annual deterministic replay、action/history/ledger完全projection、全年度checkpoint束縛、legacy保存fail-closed、definition単一入口、checkpoint完全projection、Canonical event順序、run bundleのJSON descriptor・seed役割・成否・独立implementation SHA・event container・再実行一致、台帳drawer、校正・migration・最終checkpointを回帰固定 |
...
```json
{"schemaVersion":1,"status":"pass","intent":"ready_after_confirmation","contentHead":"6e41587d976d0e02a30aef67a7aa7cc1c9c4254f","secretCandidates":0,"personalPaths":0,"origin":"pass","cleanWorktree":true,"ciConfigCount":2,"effectiveIdentity":"pass","historyMismatchCount":2,"effectiveMismatchCount":0}
```
