...
<p align="center">
  ...
  <br>
  <sub>P1 ローカル統合候補 — 代表接続を選び、投資前差分から危機寄与までを同じ画面で逆引き</sub>
</p>
...
現在は、P0 baselineに加え、代表接続1本について「何を選ぶと、接続の何が変わり、その差が危機結果へどう寄与したか」を逆引きできるP1因果縦切りを実装しています。他の19接続はschemaと表示だけで、係数・適格性は未校正です。これは研究仮説を検証できる完成MVPではありません。実行前に固定する仮説は[事前実験設計](EXPERIMENT_DESIGN.md)、状態遷移と再現条件は[シミュレーション契約](simulation-contract.md)、実測値と限界は[実行結果](RESULTS.md)がそれぞれ所有します。
...
現在のP0 baselineはルールの見える決定論モデルです。LLMは中核状態を決めません。将来エージェント推論を加える場合も、決定論エンジンと分離し、同じseedで感度を測ります。
...
- [事前実験設計](EXPERIMENT_DESIGN.md): 実行前に固定する仮説、比較条件、評価軸、反証条件
