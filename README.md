...
<p align="center">
  ...
  <br>
  <sub>P1 因果縦切り — 代表接続を選び、投資前差分から危機寄与までを同じ画面で逆引き</sub>
</p>
...
`npm run simulate` は現在の決定論コアからP0 baselineをJSONで標準出力へ生成します。`npm run verify` は、事前設計と実測結果の文書境界、test、production build、Sites packaging、baseline実行を一括確認します。
...
現在は、P0 baselineに加え、代表接続1本について「何を選ぶと、接続の何が変わり、その差が危機結果へどう寄与したか」を逆引きできるP1因果縦切りを実装しています。他の19接続はschemaと表示だけで、係数・適格性は未校正です。これは研究仮説を検証できる完成MVPではありません。仮説と反証条件の正本は[実験設計スナップショット](EXPERIMENT_DESIGN.md)、状態遷移と再現条件は[シミュレーション契約](simulation-contract.md)、実測値と限界は[実行結果](RESULTS.md)がそれぞれ所有します。設計スナップショットは初回実測に対する事前登録証拠ではありません。
...
4. 最新の因果台帳で、変更前後と投資理由を確認する。必要なら「全台帳を開く」から過去の投資へ戻る。
...
現在のM1.5は、ルールの見える決定論モデルに、固定seed・固定action表のscripted Policy Engineが3主体×3ターンのPDCAを行う層を加えています。外部APIは使わず、AI/LLM推論はまだ実装していません。状態・指標・予算・因果台帳を更新できるのは決定論エンジンだけです。
...
- [実験設計スナップショット](EXPERIMENT_DESIGN.md): 今後の実行前に固定する仮説、比較条件、評価軸、反証条件。初回実測に対する事前登録証拠ではない
