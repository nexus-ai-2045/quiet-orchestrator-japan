...
状態: **P0は公開済み / P1は人間レビュー前のローカルcommit候補**
...
公開候補として採用する前に人間が判断する項目は、初期値と施策deltaの架空係数、危機寄与selectorの重み、開示コスト等の副作用表現、画面の視覚密度である。残る機械作業は、役割同等fixtureによる国名非依存の回帰検査と、完全な因果台帳drawerである。
...
M1の技術設計には新しい人間判断は不要。次に判断が必要になるのは、P1の視覚変更案、架空係数の採用、学術参考文献の公開追加、外部AI接続、公開リポジトリへのpush・PR・merge・告知である。
