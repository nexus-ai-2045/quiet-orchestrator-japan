...
## 現在地｜M2〜M5を`main`へ統合済み

状態: **`main`はP0・P1・M1.5・M2〜M5とrun bundleまでを統合済み / drawer UIゲートはsame-HEAD検証済み / 次は反証結果の記録**

| `main`で実装済み | 残務・対象外 |
|---|---|
| 2026〜2045年の決定論的年次更新と全20接続の架空校正 | 経験的係数への置換は対象外 |
| 18主体、20接続の安定ID・共通schema・主体制約と権限分離 | 実在主体の行動推定は対象外 |
| 全接続の状態遷移、差分preview、因果台帳、全履歴drawer | 現在HEADのUI・アクセシビリティ証拠 |
| 投資先・archetype別の施策適格性と非一律delta | 係数感度と評価軸のreview収束 |
| 30日・6時間×120ターンの決定論イベント列とreplay | 契約暦・主体別観測・代替経路のreview収束 |
| A〜Eを同じengineで5 seed比較 | M6 AI説明層とM7研究候補packetは未実装 |
| 日本ノード除去後の停止接続・残存主体の試験 | 2045年最終状態への束縛確認 |
| M4/M5のevent・replayを束ねる`meta-security-run-bundle/v1` | 同一HEADのbrowser・アクセシビリティ証拠 |
...
- unit test 160件、Sites test 4件、production buildを同一content HEADで確認する。一般操作・console確認は[RESULTS](RESULTS.md)の2026-08-28 PR #4履歴証拠であり、現在branchの同一HEAD実測には数えない。因果台帳drawerのUIゲートは履歴content HEADのRESULTS証拠であり、現在`main`のsame-HEAD実測には数えない。
...
- 接続ID・表示ラベルを変えた役割同等fixtureで、表示属性だけによる差が出ない。
...
現在地: **`main`統合済み**。20接続schema、1〜3接続の配分・予算gate、台帳effect分類、接続状態由来の線表現に加え、残り19接続の目的・チャネル・ownership・初期値・施策適格性・効果倍率を[校正判断packet](docs/m2-calibration-decision-packet.md)の架空校正v1として採用した。全20接続は校正fingerprintと非一律deltaを持つ。
...
- 局面列を再生する。局面名と順序の正本は[シミュレーション契約](simulation-contract.md)であり、この文書では複製しない。
...
1. M5のA〜E、複数seed、日本除去の反証結果を`RESULTS.md`へ軸ごとに記録する。
2. `PREFLIGHT.md`・`PUBLIC_READY.md`を現在の`main`で再実行し、破棄されたbranch SHAではなくmerge commitを記録する。人間目視reviewの主張は実際に見た人が更新する。

M2校正はハッカソン用架空値として採用済みであり、経験的政策効果として扱わない。M3〜M5とrun bundleは`main`へ依存順で統合済みであり、drawerのUIゲートは`main@92f795fd`でsame-HEAD再検証済みであり、残る作業は反証結果のRESULTS記録と、人間目視reviewを含む公開判定文書の更新である。

## 現時点の人間判断

P1の視覚変更案と代表接続の架空係数v0、残り19接続の架空校正v1は採用済みである。`main`はM2〜M5とCloud handoff用run bundleまでを統合したcanonicalである。push、merge、経験的校正、外部AI接続、Cloud実行、release、告知はそれぞれ別の停止線とする。
