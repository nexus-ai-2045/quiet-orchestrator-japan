...
状態: **P0は公開済み / P1の係数v0・画面は人間採用済み、public push前のローカル候補**

| 実装済み | 未実装 |
|---|---|
| 2026〜2045年の決定論的年次更新 | 全20接続を投資可能にする校正 |
| 18主体、20接続の安定IDと共通schema | 主体ごとの能力・利害・制約 |
| `B1 ↔ C6`の状態遷移、差分preview、因果台帳 | 完全な台帳drawerと全接続の状態遷移 |
...
- 因果台帳の最新1件と、終末の1ヶ月の接続寄与からInspectorへ戻る導線を実装した。
- state version 2と旧集約状態のmigration入口を追加した。
- unit test 23件、Sites test 4件、production build、標準幅・狭幅・console・操作フローをローカルで確認した。

初期値、施策delta、危機寄与selectorの重み、開示コスト等の副作用表現、画面の視覚密度は、2026-08-24にハッカソン体験検証用v0として人間採用された。値の正本と変更ゲートは[架空係数 Calibration v0](docs/calibration-v0.md)に置く。残る機械作業は、役割同等fixtureによる国名非依存の回帰検査と、完全な因果台帳drawerである。
...
P1の視覚変更案と架空係数v0は採用済み。PR #4は公開branch上に存在するが、本SSOT統合HEADはローカルのみである。次に判断が必要になるのは、どのPRへ統合HEADを反映するかである。push、merge、学術参考文献の公開追加、経験的校正、外部AI接続、release、告知はそれぞれ別の停止線とする。
...
| P1 因果縦切り | 代表接続1本のpreview、台帳、危機寄与 | ローカル統合・検証後、人間reviewへ戻す |
